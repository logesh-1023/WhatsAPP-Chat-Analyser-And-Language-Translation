...
def webhook():
    ...
    TEXT=""
    vulgarWords=[]
    mes_word=re.split(" ",message)
    vulgarPresent=[]
    file=open('bad-words.csv')
    type(file)
    csvreader=csv.reader(file)
    for row in csvreader:
        vulgarWords.append(row[0])
    for i in mes_word:
        for j in vulgarWords:
            if i==j:
                vulgarPresent.append(j)
...
def submit():
  if request.method=='POST':
    ...
    """
    try:
        with io.open(r"K:\whatsup-chat_analyzer-and-language-Translation-main\%s"%nameoffile, "r", encoding="utf-8") as file:
    ...
    if args.stopword:
        try:
            with io.open(r"K:\whatsup-chat_analyzer-and-language-Translation-main\stop-words\%s"%stopwordinput,"r", encoding="utf-8") as file:
...
def result():
  if request.method=='POST':
    ...
    path_to_image =(r'K:\whatsup-chat_analyzer-and-language-Translation-main\source\%s'%(imgname))
    ...
    #translation
    if wordz is not None and len(wordz) > 0:
        for i in wordz:
            if i is not None and len(i.strip()) > 0:  # Check if sentence is not None or empty
                translations = translator.translate(i, dest=destlan)
                print(translations.text)
                single.append(translations.text)
